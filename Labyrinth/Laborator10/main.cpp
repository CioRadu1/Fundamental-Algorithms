...
*  Ciobanu Radu-Rares
*  Grupa 30222
    *In acest cod am adugat functiile necesare pentru anumite comenzi precum :
     -get_neighbour
     -shortest_path
     -bfs
     -generateRandomGraf
     -get_cal (care arata locurile unde se poate muta un cal pe tabela noastra dintr-un anumit punct)

     - pentru functiile get neighbour si get cal am facut o matrice de referinta pentru distantele fata de punctul curent
     - pentru functia bfs am implementat algoritmul din Corman
     - pentru generarea random a graficului am folosit o matrice de adiacenta pentru a vedea ca nu cumva sa se repete nodurile 
       iar pentru legaturi am folosit vectorul adj de la fiecare nod iar indexul ambelor noduri ce alcatuiesc o muchie este acelasi
     - pentru shortest path am apelat mai intai bfs din punctul de start iar apoi am mers din end pe drumul parintilor, iar la final am inversat 
       nodurile parcurse in path pentru a se afisea de la start la end
    ...
    do{
        ...
        if(strcmp(token, "exit") == 0 || strcmp(token, "quit") == 0 || strcmp(token, "q") == 0){
            ...
            validCommand = 1;
                ...
                strcmp(token, "bfs") == 0 || strcmp(token, "bfs_step") == 0 || strcmp(token, "cal") == 0 || strcmp(token, "bfs_tree") == 0 ||
                strcmp(token, "path") == 0){
            if(strcmp(token, "neighb") == 0 || strcmp(token, "neighbors") == 0){
                c->command = COMMAND_NEIGHB;
            }
            else if (strcmp(token, "cal") == 0) {
                c->command = COMMAND_CAL;
...
int main()
{
    ...
    for(;;){
        ...
        if(cmd.command == COMMAND_EXIT){
        ...
        else if (cmd.command == COMMAND_CAL) {
            grid2 = grid;
            Point cal[8];
            int nrPozitii = get_cal(&grid, cmd.p1, cal);
            grid2.mat[cmd.p1.row][cmd.p1.col] |= MASK_START;
            if (grid.mat[cmd.p1.row][cmd.p1.col] & MASK_WALL) {
                snprintf(err, 100, "Starting position (%d, %d) is inside a wall.", cmd.p1.row, cmd.p1.col);
                errors.push_back(err);
            ...
            for (int i = 0; i < nrPozitii; ++i) {
                if (cal[i].row >= 0 && cal[i].row < grid.rows &&
                    cal[i].col >= 0 && cal[i].col < grid.cols) {
                    grid2.mat[cal[i].row][cal[i].col] |= MASK_FILL;
                    if (grid.mat[cal[i].row][cal[i].col] & MASK_WALL) {
                        snprintf(err, 100, "Position (%d, %d) is inside a wall.", cal[i].row, cal[i].col);
                        errors.push_back(err);
                    }
                }
                else {
                    snprintf(err, 100, "Position (%d, %d) is outside the grid.", cal[i].row, cal[i].col);
                    errors.push_back(err);
                }
            }
        }
